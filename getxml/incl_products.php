...
function collectProducts($werbeflaeche, $productType)
{
	...
	while ($node_object = db_fetch_object($result)) {
		...
		if ($foundType === $productType) // Prüfe ob der gefundene Produkttyp der Anfrage entspricht
		{
			...
			// am Node fid nehmen und dann aus db - files fid::filepath nehmen (ist schon mit dem kompletten File)
			$query = "SELECT * FROM files WHERE fid='" . $banknode->field_banklogo_fid . "' LIMIT 1";
			$result4 = db_query($query);
			$filepaths = db_fetch_object($result4);
			...
			for ($i = 0; $i < sizeof($xmlArray); $i++)
			{
				...
				else if (strpos($xmlArray[$i],"<logo>") !== false)
				{
					...
					$xmlArray[$i] = createXMLTags("logo", xmlEncode("http://" . $_SERVER["SERVER_NAME"] . "/" . $filepaths->filepath));
				}
