...
#$productitems = db_query('SELECT * FROM `content_type_productitem` JOIN node using(nid) WHERE node.type="%s" AND content_type_productitem.field_proditemmybank_nid=%d', "productitem", $bank);
...
if (sizeof($returnValue) == 0) {
	...
	$productitem->bank_alias_url = $bank_alias_url;
	$productitem->bank_name = $bank_name;
