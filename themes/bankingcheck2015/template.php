...
if (theme_get_setting('pixture_superfish')) {
	drupal_add_css(drupal_get_path('theme', 'pixture_reloaded') .'/sf/css/superfish.css', 'theme', 'all', true);
 ...
 */
function bankingcheck2015_preprocess_page(&$vars, $hook) {
	global $theme;
	...
	$body_classes = array($vars['body_classes']);
		if (!$vars['is_front']) {
			...
			$body_classes[] = bankingcheck2015_id_safe('page-'. $path);
			$body_classes[] = bankingcheck2015_id_safe('section-'. $section);
			if (arg(0) == 'node') {
				if (arg(1) == 'add') {
					if ($section == 'node') {
	...
	/**
	 * Display custom template on /user as well
	 * Source: http://www.viktormiranda.com/blog/theming-drupal-user-login-page
	 */
	// Check to see that the user is not logged in and that we're on the correct page.
	if ($vars['user']->uid == 0 && arg(0) == 'user' && (arg(1) == '' || arg(1) == 'login' || arg(1) == 'register')) {
	...
	$vars['head'] .= BCTemplate::getAdditionalHeaderContent($vars);
 ...
 */
function bankingcheck2015_preprocess_node(&$vars, $hook) {
	global $user;
	...
	$vars['subleft'] = theme('blocks', 'subleft');

	# https://www.drupal.org/node/136647
 ...
 */
function bankingcheck2015_preprocess_comment(&$vars, $hook) {
	global $user;
 ...
 */
function bankingcheck2015_preprocess_block(&$vars, $hook) {
	$block = $vars['block'];
 ...
 */
function bankingcheck2015_id_safe($string) {
...
drupal_add_js(drupal_get_path('theme', 'bankingcheck2015') .'/js/head.js', 'theme');


drupal_add_js(drupal_get_path('theme', 'pixture_reloaded') .'/jquery.simplemodal.1.4.2.min.js', 'theme');
#drupal_add_js(drupal_get_path('theme', 'pixture_reloaded') .'/jquery.socialshareprivacy.js', 'theme');
#drupal_add_js(drupal_get_path('theme', 'pixture_reloaded') .'/lib/functions.js', 'theme');
#drupal_add_js(drupal_get_path('theme', 'pixture_reloaded') .'/lib/boxover.js', 'theme');
#drupal_add_js(drupal_get_path('theme', 'pixture_reloaded') .'/lib/ga_script.js', 'theme');
drupal_add_js(drupal_get_path('theme', 'pixture_reloaded') .'/lib/modal_login.js', 'theme');
#drupal_add_js(drupal_get_path('theme', 'pixture_reloaded') .'/socialshareprivacy/socialshareprivacyLoader.js', 'theme');