<ul class="gallery-b">
	...
	$query = db_query("SELECT * FROM `node` JOIN content_type_productitem USING(nid) WHERE type='productitem' LIMIT 30");
	...
	$query = db_query("SELECT nid, filepath, field_banklogo_data FROM `content_type_bank` JOIN `files` ON `content_type_bank`.`field_banklogo_fid` = `files`.`fid` LIMIT 30");
	$bankenArray = array();
	while ($result = db_fetch_array($query)) {
