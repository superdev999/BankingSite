...
//require_once '';
if(ereg("/produkte/(.*)/bewertungen/(.*)/", $_SERVER['REQUEST_URI'], $m)) {
	$productItem = node_load(array("nid"=>$m[2]));
	BCTemplate::showProductitemTabs($productItem, $m[1]);
	$bank = node_load(array("nid"=>$productItem->field_proditemmybank[0]["nid"]));
	$bcVotings = new BCVotings($productItem->nid, $m[1]);
?>
<div class="node node-mine node-type-productitem greyGradient">
<h2 class="title"><a href="<?php print url("node/".$productItem->nid); ?>"><?php print $productItem->title; ?></a></h2>
...
echo "<p>Es $verb zu diesem Produkt $count $name vorhanden.</p>";
?>
			</div>			
		</div>
	</div>
